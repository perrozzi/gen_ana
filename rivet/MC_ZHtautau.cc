...
namespace Rivet {
  ...
  public:
    ...
    void init() {
        ...
        addProjection(fs, "fs");
        // find Zee
        ZFinder zeefinder(fs, -5.0, 5.0, 0.0*GeV, ELECTRON, 65*GeV, 115*GeV, 0.2, true, true);
        addProjection(zeefinder, "zeefinder");
        // fish out Zee
        VetoedFinalState zmminput;
        zmminput.addVetoOnThisFinalState(zeefinder);
        // find Zmm
        ZFinder zmmfinder(zmminput, -5.0, 5.0, 0.0*GeV, MUON, 65*GeV, 115*GeV, 0.2, true, true);
        ...
        for (size_t i=0; i < njets; ++i) {
            ...
            _h_log10_d.push_back(bookHistogram1D(dname.str(), nbins, 0.2, 4));
            stringstream ptname;
            ptname << "aJet_pT" << i;
            _h_ajets_pT.push_back(bookHistogram1D(ptname.str(),logspace(nbins,5,300)));
            stringstream etaname;
            etaname << "aJet_eta" << i;
            _h_ajets_eta.push_back(bookHistogram1D(etaname.str(),nbins,-5,5));
        }

        // Z properties
        _h_Z_mass = bookHistogram1D("Z_mass",nbins,51,131);
        _h_Z_eta = bookHistogram1D("Z_eta",nbins,-5,5);
        _h_Z_pT = bookHistogram1D("Z_pT",logspace(nbins,5,300));

        // dijet properties
        _h_dijet_mass = bookHistogram1D("dijet_mass",nbins,60,140);
        _h_dijet_eta = bookHistogram1D("dijet_eta",nbins,-5,5);
        _h_dijet_pT = bookHistogram1D("dijet_pT",logspace(nbins,5,300));

        // H+Z properties
        _h_HZ_eta = bookHistogram1D("HZ_eta",nbins,-5,5);
        _h_HZ_pT = bookHistogram1D("HZ_pT",logspace(nbins,5,300));

        // higgs jets
        for (size_t i=0; i < 2; ++i) {
            stringstream etaname;
            etaname << "h_dau_eta" << i;
            _h_dau_eta.push_back(bookHistogram1D(etaname.str(),nbins,-5,5));
            stringstream ptname;
            ptname << "h_dau_pT" << i;
            _h_dau_pT.push_back(bookHistogram1D(ptname.str(),logspace(nbins,5,300)));
        }
    ...
    void analyze(const Event& event) {
        ...
        for (size_t i = 0; i < alljets.size(); ++i) {
            if (alljets[i].containsBottom()) {
                ...
                if (alljets[i].momentum().pT() > jets_pt_cut) nb++;
            }
        }
        ...
        for (size_t i = 0; i < alljets.size(); ++i) {
            if (i != idx0 && i != idx1){
                ...
                if (alljets[i].momentum().pT() > jets_pt_cut) na++;
            }
        }
