...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      etaRanges_lep.push_back(make_pair(-2.47, 2.47));

      DressedLeptons dressed_electrons(photon_fs, electron_fs, 0.1, true ,etaRanges_lep ,25*GeV, false);
      addProjection(dressed_electrons, "DressedElectrons");

      DressedLeptons dressed_muons(photon_fs, muon_fs, 0.1, true, etaRanges_lep ,25*GeV, false);
    ...
    void analyze(const Event& event) {
      ...
      const Jets& jets = applyProjection<FastJets>(event, "Jets").jetsByPt(25.0);
      foreach(const Jet& j, jets) {
        if (j.abseta() > 4.4) continue;
